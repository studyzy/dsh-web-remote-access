/**
 * dsh-web-remote-access/webserver — a fork of `@deepseek-ai/dsh-host-webserver` that
 ...
 */
function tokenEquals(candidate, expected) {
...
function cookieToken(req) {
    ...
    for (const part of header.split(';')) {
        ...
        if (eq === -1)
            continue;
        if (part.slice(0, eq).trim() === WEB_TOKEN_COOKIE)
 ...
 */
function webTokenGate(req, res, expected) {
    if (tokenEquals(cookieToken(req), expected))
        return 'authorized';
    const pathname = new URL(req.url ?? '/', 'http://x').pathname;
    if ((req.method === 'GET' || req.method === 'HEAD') && PUBLIC_STATIC_PATHS.has(pathname))
        return 'authorized';
...
export class WebServer extends Service {
    ...
    async [Service.init]() {
        const handle = async (req, res) => {
            ...
            if (this.config.webToken && webTokenGate(req, res, this.config.webToken) !== 'authorized')
                return;
            /* v8 ignore next -- `?? '/'` arm: node:http always sets url on server
            requests; the field is only optional on the client-side IncomingMessage type */
            const rawPath = new URL(req.url ?? '/', 'http://x').pathname;
            // The harness pins the configuration plane (settings/credentials/
            // agentPreset/host.*/llm.discoverModels) to loopback "until a real
            // authentication layer exists" (client-connection PRIVILEGED_METHODS).
            // The token gate IS that layer: every /api request here passed cookie
            // authentication, so present it to the downstream trust fence as a
            // loopback authority instead of the LAN address it arrived on. The
            // cross-site defenses (Origin/Sec-Fetch-Site) still run — the gate has
            // already admitted only cookie-bearing same-site clients.
            if (this.config.webToken && rawPath.startsWith('/api/')) {
